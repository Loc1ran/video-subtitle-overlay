...
export const LOCALIZED_PROMPTS: Record<string, PromptTemplate> = {
  vi: {
    ...
    thinkingOutput:
      'Sau khi suy nghĩ, chỉ xuất kết quả cuối cùng theo đúng định dạng mảng JSON hợp lệ duy nhất:\nĐịnh dạng mẫu: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "Bạn là chuyên gia dịch phụ đề video hàng đầu. Hãy dịch các câu thoại video sau đây sang tiếng Việt một cách tự nhiên, mượt mà và đúng ngữ điệu giao tiếp đời thường.\nGiữ giọng điệu tự nhiên, chân thực và phù hợp với nhịp độ hội thoại trong video.",
    standardFormat:
      'QUAN TRỌNG: Chỉ phản hồi duy nhất một mảng JSON hợp lệ, trong đó mỗi phần tử gồm "id" (số) và "translated" (chuỗi).\nĐịnh dạng mẫu: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "Các câu thoại gốc cần dịch:",
  },
  en: {
    thinkingHeader:
      "[THINKING & DEEP REASONING MODE ACTIVE]\nPlease think step-by-step and thoroughly reason through before producing the translations:",
    thinkingRules: [
    ...
    thinkingOutput:
      'After your reasoning, output ONLY the final valid JSON array format:\nExample format: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "You are an expert video subtitle translator. Translate the following spoken video dialogue lines accurately and naturally into English.\nKeep the tone natural, colloquial, and faithful to conversational rhythm.",
    standardFormat:
      'IMPORTANT: Respond ONLY with a valid JSON array where each object has "id" (number) and "translated" (string).\nExample format: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
  ...
  zh: {
    ...
    thinkingOutput:
      '思考完成后，仅输出最终符合规范的 JSON 数组：\n示例格式: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "你是一位精通视频字幕本地化的资深翻译专家。请将以下视频对话自然、流畅、准确地翻译为中文。\n语言风格贴近真实生活口语，契合视频交流的自然节奏。",
    standardFormat:
      '重要提示：仅返回一个合法的 JSON 数组，每个对象包含 "id"（数字）和 "translated"（字符串）。\n示例格式: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
  ...
  ja: {
    ...
    thinkingOutput:
      '思考プロセスの後、最終的な有効な JSON 配列のみを出力してください：\n出力例: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "あなたは映像字幕のプロフェッショナルです。以下の動画台詞を、自然で親しみやすい日本語に翻訳してください。\n会話のリズムを生かし、日常会話として違和感のない表現を心がけてください。",
    standardFormat:
      '重要：各要素が "id"（数値）と "translated"（文字列）を持つ有効な JSON 配列のみを出力してください。\n出力例: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
  ...
  ko: {
    ...
    thinkingOutput:
      '생각 과정을 거친 후, 오직 최종적인 유효한 JSON 배열만 출력해 주세요:\n예시 형식: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "당신은 최고의 영상 자막 번역 전문가입니다. 다음 영상 대사를 자연스럽고 생생한 한국어로 번역해 주세요.\n일상 구어체에 맞추어 생생하고 매끄럽게 대화 흐름을 살려 번역합니다.",
    standardFormat:
      '중요: 각 객체가 "id"(숫자)와 "translated"(문자열)로 구성된 유효한 JSON 배열만 응답해야 합니다.\n예시 형식: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "번역할 원문 대사:",
  },
  es: {
    thinkingHeader:
      "[MODO DE PENSAMIENTO Y RAZONAMIENTO PROFUNDO ACTIVO]\nPor favor, piensa paso a paso y analiza a fondo el contexto antes de producir las traducciones:",
    thinkingRules: [
    ...
    thinkingOutput:
      'Tras tu razonamiento, genera ÚNICAMENTE el formato de array JSON válido final:\nFormato de ejemplo: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "Eres un experto traductor de subtítulos de video. Traduce las siguientes líneas de diálogo de video de manera precisa y natural al español.\nMantén un tono coloquial, natural y fiel al ritmo de la conversación.",
    standardFormat:
      'IMPORTANTE: Responde ÚNICAMENTE con un array JSON válido donde cada objeto tenga "id" (número) y "translated" (cadena).\nFormato de ejemplo: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "Líneas originales para traducir:",
  },
  fr: {
    thinkingHeader:
      "[MODE DE PENSÉE ET RAISONNEMENT PROFOND ACTIVÉ]\nVeuillez réfléchir étape par étape et analyser soigneusement le contexte avant de traduire :",
    thinkingRules: [
    ...
    thinkingOutput:
      'Après réflexion, produisez UNIQUEMENT le tableau JSON valide final :\nExemple de format : [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "Vous êtes un traducteur expert de sous-titres vidéo. Traduisez fidèlement et naturellement les lignes de dialogue suivantes en français.\nConservez un ton naturel, vivant et fidèle au rythme de la conversation.",
    standardFormat:
      'IMPORTANT : Répondez UNIQUEMENT avec un tableau JSON valide où chaque objet contient "id" (nombre) et "translated" (chaîne).\nExemple de format : [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "Lignes originales à traduire :",
  },
  de: {
    thinkingHeader:
      "[TIEFEN-DENK- UND REFLEXIONSMODUS AKTIV]\nBitte denke Schritt für Schritt nach und analysiere den Kontext gründlich, bevor du übersetzt:",
    thinkingRules: [
    ...
    thinkingOutput:
      'Gib nach deiner Überlegung NUR das finale, gültige JSON-Array-Format aus:\nBeispielformat: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "Du bist ein professioneller Untertitel-Übersetzer. Übersetze die folgenden Dialogzeilen präzise und natürlich ins Deutsche.\nHalte den Tonfall authentisch, umgangssprachlich und lebendig.",
    standardFormat:
      'WICHTIG: Antworte AUSSCHLIESSLICH mit einem gültigen JSON-Array, in dem jedes Objekt "id" (Zahl) und "translated" (Text) enthält.\nBeispielformat: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "Zu übersetzende Originalzeilen:",
  },
  ru: {
    thinkingHeader:
      "[РЕЖИМ ГЛУБОКОГО МЫШЛЕНИЯ И РАССУЖДЕНИЯ ВКЛЮЧЕН]\nПожалуйста, рассуждайте пошагово и глубоко проанализируйте контекст перед переводом:",
    thinkingRules: [
    ...
    thinkingOutput:
      'После рассуждения выведите ТОЛЬКО итоговый валидный JSON-массив:\nПример формата: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "Вы — первоклассный переводчик видеосубтитров. Переведите следующие реплики из видео на русский язык максимально точно и естественно.\nСохраняйте живой разговорный тон и ритм естественной речи.",
    standardFormat:
      'ВАЖНО: Отвечайте ТОЛЬКО валидным JSON-массивом, где каждый объект содержит "id" (число) и "translated" (строка).\nПример формата: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "Оригинальные строки для перевода:",
  },
  th: {
    thinkingHeader:
      "[เปิดโหมดการคิดและใช้เหตุผลเชิงลึก]\nโปรดคิดทีละขั้นตอนและพิจารณาบริบทอย่างละเอียดก่อนสร้างคำแปล:",
    thinkingRules: [
    ...
    thinkingOutput:
      'หลังจากคิดวิเคราะห์แล้ว ให้ตอบกลับเฉพาะอาร์เรย์ JSON ที่ถูกต้องเท่านั้น:\nรูปแบบตัวอย่าง: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "คุณเป็นผู้เชี่ยวชาญด้านการแปลซับไตเติลวิดีโอ โปรดแปลประโยคบทสนทนาต่อไปนี้เป็นภาษาไทยอย่างเป็นธรรมชาติและลื่นไหล\nใช้น้ำเสียงที่เป็นกันเอง เป็นภาษาพูดที่สมจริงและเข้ากับจังหวะของวิดีโอ",
    standardFormat:
      'สำคัญ: ตอบกลับเฉพาะอาร์เรย์ JSON ที่ถูกต้อง โดยแต่ละออบเจกต์มี "id" (ตัวเลข) และ "translated" (ข้อความ)\nรูปแบบตัวอย่าง: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "ประโยคต้นฉบับที่ต้องแปล:",
  },
  id: {
    thinkingHeader:
      "[MODE BERPIKIR & PENALARAN MENDALAM AKTIF]\nSilakan berpikir langkah demi langkah dan pertimbangkan konteks secara mendalam sebelum menerjemahkan:",
    thinkingRules: [
    ...
    thinkingOutput:
      'Setelah bernalar, tampilkan HANYA format array JSON yang valid:\nContoh format: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    standardDesc:
      "Anda adalah penerjemah subtitle video profesional. Terjemahkan dialog video berikut ke dalam bahasa Indonesia yang alami dan luwes.\nPertahankan nada percakapan yang santai, hidup, dan sesuai dengan ritme video.",
    standardFormat:
      'PENTING: Jawab HANYA dengan array JSON yang valid di mana setiap objek memiliki "id" (angka) dan "translated" (string).\nContoh format: [{"id": 1, "translated": "..."}, {"id": 2, "translated": "..."}]',
    originalLinesHeader: "Baris dialog asli yang akan diterjemahkan:",
  },
};

export function buildLocalizedAiPrompt(
